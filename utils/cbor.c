...
/*
	...
	item is the CBOR map from the serialized blob containing the authenticator data
	f is the parser (probably parse_authkey function)
...
/*
	blob here is the full CBOR frame received from the authenticator.
	arg is the es256_pk_t variable that will be the final parsed key
	parser is the method that will try to parse each member of the CBOR map
*/
int
    ...
    int(*parser)(const cbor_item_t *, const cbor_item_t *, void *))
{
	...
	if ((item = cbor_load(blob + 1, blob_len - 1, &cbor)) == NULL) {
		printf("Not a CBOR structure (authenticator sent code 0x%02x)!\n", blob[0]);
		fflush(stdout);
		r = FIDO_ERR_RX_NOT_CBOR;
		goto fail;
	}

	if (cbor_isa_map(item) == false || cbor_map_is_definite(item) == false) {
		printf("FAIL! CBOR sent by authenticator was invalid!\n");
	...
	//arg here is the token PK for DH
...
cbor_item_t *
cbor_encode_change_pin_auth(uint8_t prot, const fido_blob_t *secret,
...
cbor_item_t *
cbor_encode_pin_auth(uint8_t prot, const fido_blob_t *secret,
...
static int cbor_add_uint8(cbor_item_t *item, const char *key, uint8_t value)
{
	struct cbor_pair pair;
	int ok = -1;
...
static int cbor_encode_largeblob_key_ext(cbor_item_t *map)
{
	if (map == NULL ||
...
static int cbor_encode_hmac_secret_param(const fido_dev_t *dev, cbor_item_t *item, const fido_blob_t *ecdh, const es256_pk_t *pk, const fido_blob_t *salt)
{
	...
	if ((enc = fido_blob_new()) == NULL ||
	    aes256_cbc_enc(fido_dev_get_pin_protocol(dev), ecdh, salt, enc) < 0) {
	...
	if ((argv[0] = es256_pk_encode_mod(pk, 1)) == NULL ||
	    (argv[1] = fido_blob_encode(enc)) == NULL ||
	    (argv[2] = cbor_encode_pin_auth(fido_dev_get_pin_protocol(dev), ecdh, enc)) == NULL ||
...
int parse_authkey(const cbor_item_t *key, const cbor_item_t *val, void *arg)
{
	...
	return (es256_pk_decode_mod(val, authkey));
}
...
static int decode_user_entry(const cbor_item_t *key, const cbor_item_t *val, void *arg)
{
...
int cbor_decode_pubkey(const cbor_item_t *item, int *type, void *key)
{
	...
	switch (*type) {
	case COSE_ES256:
		if (es256_pk_decode_mod(item, key) < 0) {
			fido_log_debug("%s: es256_pk_decode", __func__);
			return (-1);
		}
		break;
	case COSE_RS256:
		if (rs256_pk_decode_mod(item, key) < 0) {
			fido_log_debug("%s: rs256_pk_decode", __func__);
			return (-1);
		}
		break;
	case COSE_EDDSA:
		if (eddsa_pk_decode_mod(item, key) < 0) {
...
static int decode_attcred(const unsigned char **buf, size_t *len, int cose_alg, fido_attcred_t *attcred)
{
	cbor_item_t		*item = NULL;
	struct cbor_load_result	 cbor;
	uint16_t		 id_len;
...
static int decode_cred_extension(const cbor_item_t *key, const cbor_item_t *val, void *arg)
{
...
static int decode_cred_extensions(const unsigned char **buf, size_t *len,
...
int cbor_decode_cred_authdata(const cbor_item_t *item, int cose_alg,
...
static int decode_x5c(const cbor_item_t *item, void *arg)
{
	fido_blob_t *x5c = arg;
...
static int decode_attstmt_entry(const cbor_item_t *key, const cbor_item_t *val, void *arg)
{
...
static int decode_cred_id_entry(const cbor_item_t *key, const cbor_item_t *val, void *arg)
{
	fido_blob_t	*id = arg;
	char		*name = NULL;
	int		 ok = -1;
...
static int decode_assert_extension(const cbor_item_t *key, const cbor_item_t *val, void *arg)
{
...
static int decode_assert_extensions(const unsigned char **buf, size_t *len, fido_assert_extattr_t *authdata_ext)
{
	cbor_item_t		*item = NULL;
	struct cbor_load_result	 cbor;
	int			 ok = -1;
