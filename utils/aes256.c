...
int
get_random(void *buf, size_t len)
{
	int	fd = -1;
	int	ok = -1;
	ssize_t	r;
	...
	if ((fd = open(FIDO_RANDOM_DEV, O_RDONLY)) < 0)
		goto fail;
	if ((r = real_read(fd, buf, len)) < 0 || (size_t)r != len)
		goto fail;
    ...
    fido_blob_t *out, int encrypt)
{
	...
	if (secret->len != 64) {
		//fido_log_debug("%s: invalid secret len %zu", __func__, secret->len);
		return -1;
	}
	if (in->len < sizeof(iv)) {
		//fido_log_debug("%s: invalid input len %zu", __func__, in->len);
    ...
    int encrypt)
{
	...
	if (nonce->len != 12 || key->len != 32 || aad->len > UINT_MAX) {
		//fido_log_debug("%s: invalid params %zu, %zu, %zu", __func__, nonce->len, key->len, aad->len);
		goto fail;
	}
...
{
	return protocol == 2 ? aes256_cbc_fips(secret,
